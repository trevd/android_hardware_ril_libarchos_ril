...
static int setupPPP(RIL_Token t,const char* ctxid,const char* user,const char* pass)
{
	...
	*/
	...
	if (user == NULL || user[0] == 0 ||
		...
		fmt = "/system/bin/pppd %s 115200 unit 1 linkname datakey "
		"crtscts usepeerdns noauth user %s password %s defaultroute noipdefault "
		"ipcp-accept-local ipcp-accept-remote ipcp-max-failure 30 lcp-echo-interval 5 "
		"lcp-echo-failure 30 modem dump debug kdebug 8 connect "
		" \"/system/xbin/chat -v -t 500 -T *99***1# -f /system/etc/chatscripts/datakey_start\"";
...
ALOGI("Waiting For Property");
	if (wait_for_property("net.ppp1.local-ip", NULL, 50,0) < 0) {
		ALOGE("Timeout waiting net.%s.local-ip - giving up!\n",PPP_IFACE);
		//.goto error;
	}
